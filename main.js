...
const makeRender = (t=0) => {
  ...
  debris.step(TIME_STEP)
    debrisFull.forEach(deb => deb.step(TIME_STEP))
...
const light = new THREE.DirectionalLight("#aaaaaa", 1.6);
...
class InterceptorSatellite{
  constructor(x, y, z, theta, phi, psi, size){
    ...
    this.fabrixBackMain.position.set(0.0*size, -1*size, 0.4*size)
...
const INTERCEPTOR_POS = 0;
const interceptor = new InterceptorSatellite(0, 0, 0, -Math.PI/2, 0, Math.PI/2, 2)
...
setInterval(() => {
    if(Math.random() > 0.9){
  debrisFull.push(new Debris(-100, 50*Math.random() - 25, 50*Math.random() - 25, 60 + 80*Math.random(), Math.random(), Math.random(), 1, 0.5))
    }
}, 20)
debris = new Debris(-100, 0, 0, 100, 0, 0, 1, 1)
